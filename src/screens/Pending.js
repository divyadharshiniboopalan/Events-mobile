...
const Pending = (props) => {
  const UpcomingEvents = props.route.params.upcomingEventsData;
  const theme = useTheme();
  const navigation = useNavigation();
  const scrollViewRef = useRef();

  let eventsUpcomingDatas = UpcomingEvents.reduce(
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.colors.elevation.level0 }}>
      <TitleBar title={"Pending Events"} onClick={() => navigation.goBack()} />
      <View style={{ flex: 8 }}>
        <View style={{}}>
          <SectionList
            ref={scrollViewRef}
                      ...
                      >
                        ...
                        <RNText
                          title={`${moment(item.startDate).format(
                            "DD-MMM-YYYY"
                          )} to ${moment(item.endDate).format("DD-MMM-YYYY")}`}
                          color={theme.colors.onBackground}
                          fontSize={13}
                        />
                      </View>
