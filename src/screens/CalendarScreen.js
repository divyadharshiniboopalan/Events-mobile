...
const CalendarScreen = () => {
  ...
  const getEventId = async (calendars) => {
    ...
    let all = await Promise.all(
      calendars.map(async (ele) => {
        let events = await Calendars.getEventsAsync(
          [ele.id],
  ...
  const getCalendarData = async () => {
    try {
      ...
      const calendarFilteredData = newCalendatData.filter(
        (ele) => ele.allDay == false
      );
      console.log(
        "🚀 ~ getCalendarData ~ calendarFilteredData:",
        JSON.stringify(calendarFilteredData)
      );
  ...
  const getcalendarlistings = (
    date = moment(new Date()).format("YYYY-MM-DD")
  ) => {
    let newDatas = {
      [moment(date).format("YYYY-MM-DD")]: { color: "green", selected: true },
    };
    setAllData({ ...markedData, ...newDatas });

    let newData = calendarData.filter(
      (value) =>
  ...
  let eventsUpcomingDatas = calendarData
    .sort(function (a, b) {
    ...
    .reduce((prev, currentValue, index) => {
      let tempData = prev.find(
        (ele) =>
          moment(ele.title).format("YYYY-MM-DD") ==
          moment(currentValue.startDate).format("YYYY-MM-DD")
      );
      if (tempData) {
    ...
    <View style={{ flex: 1, backgroundColor: theme.colors.elevation.level0 }}>
      ...
      >
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            calendarToggle(), getcalendarlistings();
          }}
              ...
              <ScrollView style={{ height: "100%" }}>
                <View>
                  <Calendar
                    ...
                    theme={{
                      ...
                      textDayHeaderFontFamily: "Roboto_Slab",
                      textDayFontFamily: "Roboto_Slab",
                      textMonthFontFamily: "Roboto_Slab",
                    }}
                    onDayPress={(item) => {
                      getcalendarlistings(item.dateString),
                        setCalendarDate(item.dateString);
                    }}
                  />
                </View>
                ...
                >
                  ...
                  >
                    <View>
                      <RNText
                        title={moment(calendarDate).format("ddd")}
                        color={theme.colors.primary}
                        fontSize={13}
                      />
                    </View>
                    <RNText
                      title={moment(calendarDate).format("DD")}
                      color={theme.colors.primary}
                      fontSize={15}
                      fontWeight="500"
                    />
                                  ...
                                  <Menu.Item
                                    ...
                                    onPress={() => {
                                      showVirtualDeleteDialog(),
                                        ...
                                        closeMenu(),
                                        setTitle(item.title),
                                        setEventDate(new Date(item.startDate));
                                    }}
                                    title="Delete"
                ...
                renderSectionHeader={({ section: { title } }) => (
                  ...
                  >
                    ...
                    >
                      <View>
                        <RNText
                          title={moment(title).format("ddd")}
                          color={theme.colors.primary}
                          fontSize={13}
                        />
                      </View>
                      <RNText
                        title={moment(title).format("DD")}
                        color={theme.colors.primary}
                        fontSize={15}
                        fontWeight="500"
                      />
