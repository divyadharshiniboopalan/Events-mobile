...
const Completed = (props) => {
    const pastEventsData = props.route.params.pastEventsData
    const theme = useTheme()
    const navigation = useNavigation()
    const scrollViewRef = useRef()

    let eventsUpcomingDatas = pastEventsData.reduce((prev, currentValue, index) => {
    ...
    return (
        <View style={{ flex: 1, backgroundColor: theme.colors.elevation.level0 }}>
            <TitleBar title={"Completed Events"} onClick={() => navigation.goBack()} />
