...
import { Card, Menu, useTheme, Entypo, Divider, Portal, Dialog, Button } from 'react-native-paper'
...
export default function Events() {
  ...
  const getEventIdHolidays = async (calendars) => {
    ...
    let events = await Calendar.getEventsAsync(allEvents, new Date(2023, 0, 1, 0, 0, 0, 0), new Date(2035, 0, 1, 0, 0, 0, 0))
    return events
  }
  ...
  const getCalendarData = async () => {
    try {
      ...
      setupcomingEvents(upcomingEventsData.sort(function (a, b) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.colors.elevation.level0 }}>
      ...
      <View style={{ flex: 0.7, flexDirection: "row", alignItems: "center", paddingHorizontal: 20, backgroundColor: "#fbede3", justifyContent: "space-between" }}>
        ...
        <RNText title={"Events"} fontSize={20} color={theme.colors.primary} variant="bodyMedium" fontWeight="500" />
        <TouchableOpacity style={{ marginHorizontal: 5 }} onPress={() => navigation.navigate("CalendarScreen")}>
          <FontAwesome5 name="calendar-day" size={18} color={theme.colors.primary} />
        </TouchableOpacity>
      </View>

      <View style={{ height: scr_height * 0.08, backgroundColor: "#f3f3f3", width: "90%", borderRadius: 15, alignSelf: "center", justifyContent: "space-around", alignItems: "center", flexDirection: "row", padding: 10, marginTop: 15 }}>
        <TouchableOpacity style={{ width: "30%", backgroundColor: ChooseEventsType === "Upcoming" ? theme.colors.background : "#f3f3f3", height: scr_height * 0.06, alignSelf: "center", borderRadius: 10, justifyContent: "center" }} onPress={() => setchooseEventsType("Upcoming")} activeOpacity={1}>
          <RNText title={"UPCOMING"} fontSize={12} color={ChooseEventsType === "Upcoming" ? theme.colors.primary : "#8a8c8f"} variant="bodyMedium" fontWeight="500" textAlign="center" />
        </TouchableOpacity>
        <TouchableOpacity style={{ width: "30%", backgroundColor: ChooseEventsType === "PastEvents" ? theme.colors.background : "#f3f3f3", height: scr_height * 0.06, alignSelf: "center", borderRadius: 10, justifyContent: "center" }} onPress={() => setchooseEventsType("PastEvents")} activeOpacity={1}>
          <RNText title={"PAST EVENTS"} fontSize={12} color={ChooseEventsType === "PastEvents" ? theme.colors.primary : "#8a8c8f"} variant="bodyMedium" fontWeight="500" textAlign="center" />
        </TouchableOpacity>
        <TouchableOpacity style={{ width: "30%", backgroundColor: ChooseEventsType === "Holidays" ? theme.colors.background : "#f3f3f3", height: scr_height * 0.06, alignSelf: "center", borderRadius: 10, justifyContent: "center" }} onPress={() => setchooseEventsType("Holidays")} activeOpacity={1}>
          <RNText title={"HOLIDAYS"} fontSize={12} color={ChooseEventsType === "Holidays" ? theme.colors.primary : "#8a8c8f"} variant="bodyMedium" fontWeight="500" textAlign="center" />
        </TouchableOpacity>
      </View>
            ...
            {listingLoader ?
              ...
              <FlatList
                ...
                ListEmptyComponent={
                  <View>
                    ...
                    <View style={{ alignItems: "center" }}>
                      <RNText title={`No ${ChooseEventsType} Event`} fontSize={20} color={theme.colors.onBackground} variant="bodyMedium" fontWeight="500" />
                ...
                renderItem={({ item, index }) => (
                  ...
                  <Card style={{ width: "90%", backgroundColor: "white", margin: 5, alignSelf: "center", borderLeftWidth: 5, borderColor: item.color }}>
                    <Card.Content>
                      <View style={{ flexDirection: "row", width: "100%", alignItems: "center" }}>
                        <View style={{ width: "20%" }}>
                          <Image source={require("../assests/3dicon.png")} style={{ width: 40, height: 40 }} resizeMode={"cover"} />
                        </View>
                        <View style={{}}>
                          <View style={{ marginBottom: 8, flexDirection: "row" }}>
                            ...
                            <View>
                              ...
                              >
                                ...
                                <Divider />
                                <Menu.Item onPress={() => { showVirtualDeleteDialog(), setupdateEventId(item.id), closeMenu(), setTitle(item.title) }} title="Delete" titleStyle={{ color: "red" }} leadingIcon={() => <MaterialCommunityIcons name="trash-can-outline" size={21} color="red" />} />
                              </Menu>
          ...
          <View style={{ marginTop: 10 }}>
            ...
            {listingLoader ?
              ...
              <FlatList
                ...
                renderItem={({ item, index }) => (
                  item.allDay == false || item.organizerEmail != "addressbook#contacts@group.v.calendar.google.com" &&
                  <View>
                    <Card style={{ width: "90%", backgroundColor: "white", margin: 5, borderLeftWidth: 5, borderColor: "#33b679", alignSelf: "flex-end", marginHorizontal: 20 }}>
                      <Card.Content>
                        <RNText title={item.title} color={theme.colors.primary} fontSize={15} fontWeight="500" />
                        <View style={{ flexDirection: "row", alignItems: "center", marginTop: 5, columnGap: 6 }}>
                          <MaterialIcons name="event" size={15} color="black" />
                          <RNText title={moment(item.startDate).format("DD-MMM-YYYY")} fontSize={13} />
                        </View>
                      </Card.Content>
                    </Card>
                  </View>
      ...
      <Portal>
        <Dialog visible={DeleteDailogVisible} onDismiss={hideDeleteDialog} style={{ backgroundColor: "white", }}>
          <Dialog.Title style={{fontFamily:"Roboto_Slab"}}>Alert</Dialog.Title>
          <Dialog.Content>
